...
public class SwerveSetpointGenerator {
  ...
  private static final SwerveModuleState ZERO_MODULE_STATE =
      new SwerveModuleState(0, GeomUtil.ROTATION2D_ZERO);
  ...
  private SwerveSetpoint prevSetpoint =
      new SwerveSetpoint(
          ...
          new SwerveModuleState[] {
            ZERO_MODULE_STATE, ZERO_MODULE_STATE, ZERO_MODULE_STATE, ZERO_MODULE_STATE
          });
  ...
  /**
   * Set the disabled flag, which will cause the next call to generateSetpoint() to skip the
   * optimizations and constraint considerations.
   *
   * @param disabled True to disable the optimizations and constraints. Default is false.
   */
  ...
  public SwerveSetpoint generateSetpoint(ChassisSpeeds desiredState) {
    ...
    //       desiredState, swerveDriveKinematics.toSwerveModuleStates(desiredState));
    // }
    return generateSetpointInner(desiredState, 0.02);
  }

  private SwerveSetpoint generateSetpointInner(ChassisSpeeds desiredState, double dt) {
    final int moduleCount = DriveConstants.MODULE_TRANSLATIONS.length;
    ...
    if (moduleLimits.maxDriveVelocity() > 0.0) {
      SwerveDriveKinematics.desaturateWheelSpeeds(
          desiredModuleState, moduleLimits.maxDriveVelocity());
